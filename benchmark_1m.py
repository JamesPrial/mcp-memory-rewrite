...
# Add e2e_tests to path
sys.path.insert(0, str(Path(__file__).parent / "e2e_tests"))
...
def main():
    ...
    for batch_start in range(0, 1000000, batch_size):
        ...
        start = time.time()
        result = client.tools_call("create_entities", {"entities": batch_entities})
        ...
        if (batch_end % 100000) == 0:
            ...
            print(f"  {batch_end:,} entities | {total_time:.1f}s | {rate:.0f} entities/sec")
    ...
    for i in range(100):
        ...
        start = time.time()
        result = client.tools_call("search_nodes", {"query": search_term})
        elapsed = time.time() - start
